...
public interface LogisticsMapper {
	...
	@Insert({
        "<script>",
        "insert into `msyb_resource`.`msyb_logistics` (",
        "	open_id, level_id, ",
        "	name, mobile, ",
	...
	int insert(@Param("p") Logistics logistics);
	
	@Update({
        "<script>",
        "update `msyb_resource`.`msyb_logistics` ",
        "<set>",
        "<if test='p.logisticsCode != null'>",
        "`logistics_code` = #{p.logisticsCode},",
        "</if>", 
        "<if test='p.transactionId != null'>",
	...
	int updateByPrimaryKey(@Param("p") Logistics logistics);
	
	@Select({
        "<script>",
        "select ",
        "	logistics_code logisticsCode	",
        "from `msyb_resource`.`msyb_logistics` ",
        "where 1=1 ",
        "<if test='p.openId != null'>",
        "	AND `open_id` = #{p.openId,jdbcType=VARCHAR} ",
        "</if>",
        "<if test='p.levelId != null'>",
        "	AND `level_id` = #{p.levelId,jdbcType=VARCHAR} ",
        "</if>",
        " order by create_time desc ",
        "</script>"
	})
	List<Logistics> selectBy(@Param("p") Logistics logistics);
	//******************************************************************************************************************************************
	@Select({
            "<script>",
            "select ",
